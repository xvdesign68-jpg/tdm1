...
export function kpiAgg(docs, leads, nowMs, brand) {
  ...
  (leads || []).forEach(l => { if (!l) return; if (brand && String(l.brand || '') !== brand) return; const det = toMs(l.detected_at); if (!det) return; const day = vnDay(det); if (day < d0 || day > t) return;
    L.all++; const tp = tempOf(l); if (tp === 'junk') { L.junk++; return; }
    /* LỆNH #42: lead vai người bán/chủ bài/tự bình luận KHÔNG phải lead hợp lệ (bộ đếm cũng không tính) */
    const rb = /^(seller|poster_self)$/.test(String(l.role || '')) || l.self_comment === true; if (rb) { L.roleBad++; return; }
    L.valid++; if (tp === 'hot') L.hot++; if (l.dropped) L.dropped++; if (l.lost) L.lost++;
    if (!l.dropped && !l.lost) L.validOpen++; });
...
if (process.argv.includes('--selftest')) {
  ...
  const ok = r.cur.new === 6 && r.cur.hot === 1 && r.cur.closed === 1 && r.cur.scanned === 15 && r.prev.new === 4 && r.delta.new === 50 && r.byBrand.a.new === 5 && r.byBrand.b.new === 1
    && r.leads.all === 4 && r.leads.valid === 2 && r.leads.hot === 1 && r.leads.junk === 1 && r.leads.dropped === 0 && r.leads.roleBad === 1 && r.leads.lost === 1 && r.leads.validOpen === 1
...
if (isMain) {
  ...
  console.log('  lead có điểm: ' + L.all + ' = hợp lệ ' + L.valid + ' (nóng ' + L.hot + ') + rác ' + L.junk + ' + vai người bán/chủ bài ' + L.roleBad + (L.valid === r.cur.new ? '   ✓ khớp ô "Lead hợp lệ"' : '   ✗ LỆCH ô "Lead hợp lệ" (' + r.cur.new + ')'));
  console.log('  trong ' + L.valid + ' lead hợp lệ: đã Loại (dropped) ' + L.dropped + ' · không thành (lost) ' + L.lost + ' → còn mở thật sự ' + L.validOpen + '  (= chip "còn mở" trên web v119-81)');
  console.log('  (ô KPI đếm theo ngày PHÁT HIỆN nên vẫn tính cả lead sau đó bị Loại/không thành — đúng nghĩa "AI đã lọc rác", không phải "còn mở"; từ LỆNH #42 lead vai người bán/chủ bài KHÔNG tính)');
}
