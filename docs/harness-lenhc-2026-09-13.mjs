...
{ const rr = run(W, [PR, '_rules_sample']); const rr2 = run(W, [PR, '_rules_sample']); const t = fs.readFileSync(path.join(W, '_rules_sample'), 'utf8');
  ok(rr.code === 0 && /PATCH OK/.test(rr.out) && rr2.code === 0 && /idempotent/.test(rr2.out) && t.includes("'dropped_by', 'dropped_reason', 'ai_feedback', 'phone', 'lost'") && (t.match(/'dropped_reason'/g) || []).length === 1, 'Rules: whitelist leads + dropped_reason/ai_feedback/phone (mốc dump #47), idempotent');
