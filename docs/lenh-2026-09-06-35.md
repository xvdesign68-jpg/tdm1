# LỆNH #35 (06/09/2026) — CHỈ ĐỌC: banner "Kênh dữ liệu outreach_stats lỗi permission-denied" trên máy Super Admin

## Bối cảnh
- Sau khi deploy zip v119-55 + worker 2026-09-06c, anh thấy banner kênh `outreach_stats` permission-denied. Banner do `snapErr('outreach_stats')` (live.js `lazyOutreach`) bắn khi listener `outreach_stats` bị Firestore từ chối; kênh này KHÔNG nằm trong nhóm tự đăng ký lại (v119-47 chỉ config/sources/notes/leads) nên banner treo tới khi F5.
- Truy vấn super: `where('day','==', <ngày VN>)` (không limit); brand user: `where('brandCode','==', brand)`. Cùng phiên, kênh `outreach_log` (orderBy at desc limit 100) và `workers` (read = isSuperAdmin) đăng ký TRƯỚC; banner hiện kênh lỗi SAU CÙNG nên chưa biết 2 kênh kia có lỗi hay không.
- Giả thuyết cần loại trừ: (1) Rules ĐANG CHẠY không phải bản trong `~/firebase-s13/firestore.rules` (deploy từ codebase khác đè — `~/codebase2` / `~/smartlead-zalo-fn` có thể có firestore.rules cũ thiếu block outreach_*); (2) block `outreach_stats` khác thiết kế (vd có `request.query.limit`); (3) tài khoản trong tab đó không phải super (role/active); (4) lỗi thoáng qua lúc token/ruleset re-evaluate.

## KHỐI 1 — `/tmp/l35_rules.mjs` (so Rules đang chạy với file local + ruleset gần đây)
```bash
cat > /tmp/l35_rules.mjs <<'EOF'
// LENH #35 (06/09/2026) — CHỈ ĐỌC: so Rules ĐANG CHẠY trên Firestore với file firestore.rules của ~/firebase-s13; liệt kê ruleset gần đây (ai deploy lúc nào);
// in block outreach_stats/outreach_log/workers/daily_stats + helper isSuperAdmin/isActiveUser/myBrand của bản ĐANG CHẠY. Không ghi gì.
import fs from 'fs'; import { execSync } from 'child_process';
const P = 'smartlead-z15';
const tok = execSync('gcloud auth print-access-token', { encoding: 'utf8' }).trim();
const get = async (path) => { const r = await fetch('https://firebaserules.googleapis.com/v1/' + path, { headers: { Authorization: 'Bearer ' + tok } }); const j = await r.json(); if (!r.ok) throw new Error(path + ' → HTTP ' + r.status + ' ' + JSON.stringify(j).slice(0, 200)); return j; };
const vn = s => s ? new Date(s).toLocaleString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' }) : '-';
const rel = await get('projects/' + P + '/releases');
const fsRel = (rel.releases || []).find(r => r.name.endsWith('/cloud.firestore'));
console.log('== RELEASE cloud.firestore ==');
console.log(' ruleset đang chạy:', fsRel ? fsRel.rulesetName.split('/').pop() : '(không thấy)', '| tạo', vn(fsRel && fsRel.createTime), '| cập nhật (lần release gần nhất)', vn(fsRel && fsRel.updateTime));
(rel.releases || []).filter(r => !r.name.endsWith('/cloud.firestore')).forEach(r => console.log(' release khác:', r.name.split('/').pop(), '→', r.rulesetName.split('/').pop(), vn(r.updateTime)));
const rs = await get('projects/' + P + '/rulesets?pageSize=8');
console.log('== 8 RULESET mới nhất (mỗi lần deploy rules = 1 ruleset) ==');
(rs.rulesets || []).forEach(r => console.log(' -', r.name.split('/').pop(), vn(r.createTime), r.name === (fsRel && fsRel.rulesetName) ? '← ĐANG CHẠY' : ''));
if (!fsRel) process.exit(1);
const full = await get(fsRel.rulesetName);
const src = ((full.source || {}).files || []).map(f => f.content).join('\n');
fs.writeFileSync(process.env.HOME + '/rules-deployed.txt', src);
const local = fs.existsSync(process.env.HOME + '/firebase-s13/firestore.rules') ? fs.readFileSync(process.env.HOME + '/firebase-s13/firestore.rules', 'utf8') : '';
console.log('== SO SÁNH == đang chạy:', src.length, 'ký tự,', src.split('\n').length, 'dòng | file local ~/firebase-s13/firestore.rules:', local.length, 'ký tự,', local.split('\n').length, 'dòng →', local && local.trim() === src.trim() ? 'GIỐNG NHAU' : 'KHÁC NHAU (xem diff bên dưới)');
const blocks = ['outreach_stats', 'outreach_log', 'workers', 'worker_config', 'daily_stats', 'system_status', 'audit_log', 'fb_accounts', 'brands', 'leads', 'config', 'notes'];
console.log('== BLOCK có trong bản ĐANG CHẠY ==');
blocks.forEach(b => { const re = new RegExp('match\\s+/' + b + '/'); console.log(' ', re.test(src) ? '✓' : '✗ THIẾU', b, '| local:', re.test(local) ? '✓' : '✗'); });
const blk = (s, name) => { const one = s.match(new RegExp('^[ \\t]*match\\s+/' + name + '/\\{[^}]*\\}\\s*\\{[^\\n]*\\}[ \\t]*$', 'm')); if (one) return one[0]; const m = s.match(new RegExp('([ \\t]*)match\\s+/' + name + '/\\{[^}]*\\}\\s*\\{[\\s\\S]*?\\n\\1\\}')); return m ? m[0] : '(không thấy)'; };
console.log('== BLOCK outreach_stats (ĐANG CHẠY) ==\n' + blk(src, 'outreach_stats'));
console.log('== BLOCK outreach_log (ĐANG CHẠY) ==\n' + blk(src, 'outreach_log'));
console.log('== BLOCK workers (ĐANG CHẠY) ==\n' + blk(src, 'workers'));
const fn = (s, name) => { const m = s.match(new RegExp('function\\s+' + name + '\\s*\\([^)]*\\)\\s*\\{[\\s\\S]*?\\n\\s*\\}')); return m ? m[0] : '(không thấy)'; };
console.log('== HELPER (ĐANG CHẠY) ==');
['isSuperAdmin', 'isActiveUser', 'myBrand', 'userExists', 'me'].forEach(n => console.log(fn(src, n)));
if (local && local.trim() !== src.trim()) {
  const a = local.split('\n'), b = src.split('\n'); const onlyLocal = a.filter(l => l.trim() && !b.includes(l)), onlyDep = b.filter(l => l.trim() && !a.includes(l));
  console.log('== DÒNG CHỈ CÓ Ở LOCAL (' + onlyLocal.length + ') ==\n' + onlyLocal.slice(0, 40).join('\n'));
  console.log('== DÒNG CHỈ CÓ Ở BẢN ĐANG CHẠY (' + onlyDep.length + ') ==\n' + onlyDep.slice(0, 40).join('\n'));
}
EOF
```
## KHỐI 2 — `~/firebase-s13/functions/_l35_db.mjs`
```bash
cat > ~/firebase-s13/functions/_l35_db.mjs <<'EOF'
// LENH #35 (06/09/2026) — CHỈ ĐỌC, đặt trong ~/firebase-s13/functions: hồ sơ user super + đếm outreach_stats hôm nay + user đang active (che email)
import admin from 'firebase-admin'; if (admin.apps.length === 0) admin.initializeApp();
const db = admin.firestore();
const mask = e => String(e || '').replace(/^(..)[^@]*(@.*)$/, '$1***$2');
const tk = new Date(Date.now() + 7 * 3600 * 1000).toISOString().slice(0, 10);
const us = await db.collection('users').get();
console.log('== USERS (' + us.size + ') ==');
us.docs.forEach(d => { const u = d.data(); console.log(' -', d.id.slice(0, 8) + '…', mask(u.email), '| role', u.role || '-', '| active', u.active, '| brand', u.brand || '-', '| provider', u.provider || '-'); });
const st = await db.collection('outreach_stats').where('day', '==', tk).get();
console.log('== outreach_stats ngày', tk, ':', st.size, 'doc ==');
st.docs.forEach(d => { const x = d.data(); console.log(' -', d.id, JSON.stringify({ brandCode: x.brandCode, day: x.day, react: x.react, comment: x.comment, friend: x.friend, inbox: x.inbox, replied: x.replied })); });
const all = await db.collection('outreach_stats').get();
const bad = all.docs.filter(d => { const x = d.data(); return !x.brandCode || !x.day; });
console.log('outreach_stats tổng', all.size, 'doc · thiếu brandCode/day:', bad.length, bad.slice(0, 5).map(d => d.id).join(', '));
EOF
```
## KHỐI 3 — chạy
```bash
cat > /tmp/l35.sh <<'EOF'
# LỆNH #35 — CHỈ ĐỌC. chạy bằng `bash /tmp/l35.sh 2>&1 | tee /tmp/l35.out`
cd ~/firebase-s13/functions || exit 1
echo "=== (a) Rules ĐANG CHẠY trên Firestore vs file local + ruleset gần đây ==="
node /tmp/l35_rules.mjs
echo "=== (b) users + outreach_stats hôm nay ==="
( set -a; . ./.env; set +a; node _l35_db.mjs )
echo "=== XONG LỆNH #35 (không ghi gì) ==="
EOF
bash /tmp/l35.sh 2>&1 | tee /tmp/l35.out
```
Kèm: F12 → Console trên tab bị banner → copy các dòng `[SmartLead] outreach_stats lỗi: …` (và `outreach_log`/`workers` nếu có) + cho biết đang đăng nhập tài khoản nào, mở tab nào lúc banner hiện.

## ★ KẾT QUẢ #35 (anh chạy 06/09/2026 ~13:30 VN) — RULES ĐÚNG, LỖI Ở WEB (banner treo sau lỗi thoáng qua)
- Ruleset đang chạy `5662a477-…` tạo 11:45:42 05/09 (= LỆNH #23) — **không có ruleset nào mới hôm nay** (8 ruleset gần nhất: 05/09, 04/09 ×3, 01/09, 30/08 ×2, 28/08). Bản đang chạy **GIỐNG HỆT** `~/firebase-s13/firestore.rules` (14611 ký tự / 317 dòng). Đủ block outreach_stats/outreach_log/workers/worker_config/daily_stats/system_status/audit_log/fb_accounts/brands/leads/config (`notes` là subcollection nên regex không bắt — không phải thiếu).
- Block `outreach_stats` = `request.auth != null && (users.role == 'superadmin' || users.brand == resource.data.brandCode)`, write false — super đọc mọi truy vấn, brand user đọc `where brandCode == brand mình` → đúng thiết kế v119-34.
- Users: super `xu***@gmail.com` role superadmin, active, brand test-agency; 8 sales (hscl-01 ×3, tts, tuyendung, test-agc-3, test-agency) + 2 pending. `outreach_stats` hôm nay: `hscl-01__2026-09-06 {react 7, comment 3, friend 1, inbox 1}` → worker 2026-09-06c đang chạy phễu thật.
- **Kết luận**: không có lý do nào ở Rules/tài khoản để Firestore từ chối `where day == hôm nay` của super. Đây là lỗi **thoáng qua** (Firestore đánh giá lại listener đang mở khi token/phiên đổi; tab mở lâu) và **web giữ banner mãi**: v119-47 tự đăng ký lại TOÀN BỘ listener sau 10 s nhưng chỉ 4 kênh (config/sources/notes/leads) gọi `chanOk()` gỡ cờ → `outreach_stats` đăng ký lại OK mà cờ `chanFailed` còn nguyên → banner đỏ treo tới khi F5 (đúng như ảnh: lead vẫn hiện, chỉ banner treo). Kênh này đăng ký SAU CÙNG nên banner luôn in tên nó.
- **Vá FE v119-56 / v120-esm-f** (`docs/fe-2026-09-06-v119-56.py`, live.js): (1) 9 kênh còn lại gọi `chanOk(<kênh>)` khi nhận dữ liệu (outreach_log · workers · outreach_stats · scans · bd_month · system_alerts · scanned_posts · brand doc · users chờ duyệt); (2) lúc đăng ký lại: xoá hết cờ + `restartPending=true` → kênh đầu tiên OK sau đó gỡ banner (kênh lazy như outreach_* chỉ đăng ký khi mở tab nên không tự gỡ được); lỗi THẬT → kênh lỗi lại → banner hiện lại (`chanRetry` chặn lặp vô hạn). Harness `docs/harness-chanok-2026-09-06.mjs` 12/12 (lỗi thoáng qua → gỡ; lỗi thật → giữ; 2 kênh lỗi → cả 2 OK mới gỡ; chanOk không cờ → im lặng). Smoke +1 check (live.min.js gọi chanOk cho ≥9 kênh).

