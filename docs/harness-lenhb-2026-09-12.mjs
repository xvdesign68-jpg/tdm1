...
{ const F0 = stub.makeDb(); const db = F0.db; const now = Date.now();
  ...
  const Lt = F0.store.get('leads/L_t__b1');
  ok(r6 === true && (Lt.touches || []).length === 3 && (F0.store.get('leads/L_t2__b1').touches || []).length === 1 && Lt.identityKey === 'u:https://www.facebook.com/profile.php?id=400' && Lt.author_uid === '400' && Lt.author_url === lead6.author_url, 'PC-4 (rà): cùng văn bản nhưng lead cũ có identityKey KHÁC người → không gộp vào lead đó (gộp vào lead không định danh cùng brand, lead đó NHẬN định danh của bản mới)');
