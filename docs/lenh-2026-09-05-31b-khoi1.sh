...
TS=$(date +%Y%m%d-%H%M%S)
for f in index.js lib/scorer.js outreach.js; do
  if grep -q 'v-selfcmt' "$f"; then echo "da co marker, giu backup cu: $f"; else cp "$f" "$f.bak-$TS"; echo "backup $f.bak-$TS"; fi
done
cat > _l31_patch.cjs <<'PATCHJS'
...
PATCHJS
node _l31_patch.cjs \
 && node --check index.js && node --check lib/scorer.js && node --check outreach.js && echo "SYNTAX OK" \
 && (set -a; . ./.env; set +a; node --input-type=module -e "const m = await import('./index.js'); const s = await import('./lib/scorer.js'); console.log('IMPORT OK · scheduledScan =', typeof m.scheduledScan, '· manualScan =', typeof m.manualScan, '· outreachTick =', typeof m.outreachTick, '· normRole =', typeof s.normRole);") \
 && echo "--- marker:" && grep -n 'v-selfcmt' index.js lib/scorer.js outreach.js | cut -c1-110 \
 && rm -f _l31_patch.cjs \
 && firebase deploy --only functions:scheduledScan,functions:manualScan,functions:outreachTick \
 && echo "DEPLOY OK" \
 && gcloud run services describe scheduledscan --region asia-southeast1 --format='value(status.latestReadyRevisionName)' 2>/dev/null | sed 's/^/scheduledscan rev: /' \
 && gcloud run services describe outreachtick --region asia-southeast1 --format='value(status.latestReadyRevisionName)' 2>/dev/null | sed 's/^/outreachtick rev: /'
echo "exit=$?"
