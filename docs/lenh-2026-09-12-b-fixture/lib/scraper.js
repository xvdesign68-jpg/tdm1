...
/* LENH #48 (12/09/2026, KHỐI 0 LỆNH B): record BrightData — bài chỉ có profile_id (uid số, KHÔNG có user_url); bình luận có commentator_profile_url kèm tracking ?__cft__… và user_id = pfbid.
...
export function normalizePost(p, source) {
  return {
    ...
    user_url: profileOf48(p.user_url || p.profile_url || p.user_profile_url || '', p.profile_id),  /* v-patch user_url-map · LENH #48: BrightData chỉ có profile_id (uid số) → profile.php?id=<uid> */
