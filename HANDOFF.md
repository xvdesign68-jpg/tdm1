...
3. ☐ **Zip frontend MỚI NHẤT** = `smartleads17deploy-v119-56.zip` (= v119-55 + banner kênh permission-denied hết treo sau lỗi thoáng qua — mọi kênh gọi chanOk; smoke 89/89; bản riêng `v120-esm-f`; `docs/fe-2026-09-06-v119-56.py`). Trước đó (anh ĐÃ deploy 06/09) `smartleads17deploy-v119-55.zip` (= v119-54 + Content Studio card "📈 Hiệu quả nội dung" + mô tả cấu trúc mới, đi cùng LỆNH #34 + worker 2026-09-06c; smoke 88/88; bản riêng `v120-esm-e`; trước đó v119-54 = v119-53 + Content Studio bỏ opt-out mặc định; trước đó v119-53 = v119-52 + Tiếp cận vẽ riêng panel VPS theo heartbeat + card "Cảnh báo hệ thống" (super) + logo login lấy ?v= từ link icon; smoke 87/87) (+ bản kế tiếp `smartleads17deploy-v120-esm-d.zip` = cùng tính năng, src/app là ES module thật — deploy sau khi v119-52 ổn; phiên mới nếu anh đưa zip v120-esm thì sửa code theo kiểu module, xem mục v120-esm trong CLAUDE.md) (= v119-50 + giờ server cho mốc ghi + pipeline diff theo cột + tách boot() live.js; smoke 81/81). Trước đó v119-50 (= v119-49 + thẻ ⛔ BrightData ngưng + chip thanh nhịp quét + ô "Nhịp gieo mỗi nguồn"; smoke 79/79). Trước đó v119-49 (= v119-48 + NHÓM 3: wizard "Thêm brand mới" + Bảng brand agency + LỆNH #17 daily_stats; trước đó v119-48 = v119-47 + thông báo đứng lại/nút Mở lead/rung + D.cfg; v119-47 = v119-46 + tự đăng ký lại listener sau lỗi kênh; v119-46 = v119-45 + thông báo hệ thống khi tab không focus; v119-45 = v119-44 + enablePush bền hơn; v119-44 = v119-43 + VAPID_KEY; trước đó `smartleads17deploy-v119-43.zip` (đã gửi anh 04/09 — v119-42 (việc nhỏ + Đợt 3 FE) + PWA/sw.js/manifest + thanh tab dưới + push FCM FE + badge Safety Score). Worker mới nhất `worker.mjs 2026-09-06b`. Đây là snapshot source đầy đủ, phiên mới giải nén ra là sửa được ngay.
4. ☑ (anh ĐÃ chép VPS 06/09) **`worker.mjs` bản `2026-09-06c`** (gửi anh 06/09 cùng LỆNH #34: `stampExtra` ghi `leads.outreach.content` = meta nội dung + `comment_at/inbox_at` cho card Hiệu quả nội dung — mục 06c `docs/worker-2026-09-06-refund-capture.md`; config.json không đổi; trước đó 2026-09-06b: comment-lead đi API nội bộ — tym + trả lời bình luận qua /api/graphql/, `config.json.graphql.commentDocId/replyDocId = 28781864408106143`; 06 = hoàn van kết bạn khi bỏ qua + `--capture` — `docs/worker-2026-09-06-refund-capture.md`; trước đó 2026-09-05c gửi 05/09: 05 = giải mã uid chủ trang + checkReplies phủ mọi lead; 05b = nhãn funnel + kiểm ngôn ngữ FB (nick phải để Tiếng Việt) + chờ vẽ + skip nội dung không xem được; 05c = trả lời comment-lead nhận ô soạn theo FOCUS/cấu trúc, nút Thích icon — `docs/worker-2026-09-05b-comment-reply.md`; trước đó 2026-09-04b Safety Score) — nếu định sửa worker.
...
## 5. Trạng thái deploy hiện tại (tính đến 06/09/2026)
- **FE:** zip mới nhất cần deploy = **v119-56** (06/09 chiều: = v119-55 + banner kênh hết treo, đi cùng LỆNH #35 chỉ đọc `docs/lenh-2026-09-06-35.md`; bản riêng v120-esm-f). Trước đó **v119-55** (06/09: = v119-54 + Content Studio card "📈 Hiệu quả nội dung" + mô tả cấu trúc mới, đi cùng LỆNH #34; bản riêng v120-esm-e; `docs/fe-2026-09-06-v119-55.py`). Trước đó **v119-54** (06/09: = v119-53 + Content Studio bỏ opt-out mặc định, đi cùng LỆNH #33; bản riêng v120-esm-d; `docs/fe-2026-09-06-v119-54.py`). Trước đó **v119-53** (06/09: panel VPS vẽ riêng + Cảnh báo hệ thống + logo ?v=; bản riêng v120-esm-c; `docs/fe-2026-09-06-v119-53.py`). Trước đó **v119-52** (05/09, đi cùng LỆNH #31: chip vai + bài gốc trong modal + nhãn Bài đã quét; bản riêng v120-esm-b). Trước đó **v119-51** (05/09: giờ server mốc ghi + pipeline diff cột + tách boot(); smoke 81/81; worker 2026-09-05 chép đè mọi VPS + bật thử `inbox.enabled` 1 VPS + LỆNH #24 `docs/lenh-2026-09-05-worker-uid.md`). Trước đó **v119-50** (05/09: banner BrightData ngưng đọc `system_status/brightdata` — cần Rules LỆNH #23; ô `scanIntervalMin`). Trước đó **v119-49** (nhóm 3: wizard brand + Bảng brand; LỆNH #17 `docs/lenh-2026-09-04-nhom3.md` ĐÃ CHẠY 04/09: CF `statsOnLead` ACTIVE + Rules `daily_stats` + backfill 73 doc/4 brand). Trước đó **v119-48** (push đã nghiệm thu chạy đúng tới Chrome Windows 04/09; điện thoại cần tự bấm Bật thông báo) (worker 2026-09-04b chép đè mọi VPS; LỆNH #7–#11 backend ĐÃ CHẠY XONG 04/09 — config/private, Rules whitelist+audit_log, index notes, backup+alert, setUserLock, push CF; VAPID public key đã nhét vào v119-44 → push FE bật được). Trước đó v119-42 (backend đi kèm: LỆNH #7–#10 chưa chạy → khoá tài khoản tạm là khoá mềm, audit_log ghi nhưng chưa đọc được, notes fallback không giới hạn, config/private chưa tách). Trước đó v119-41b (Đợt 1: số thật, Hôm nay, Hộp việc, Gọi·Zalo·SMS, tìm SĐT, chốt/không thành, modal mới, pipeline mobile, lead cũ realtime, Dừng tất cả).
