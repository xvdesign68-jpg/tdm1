...
export async function prefilterLead(post, brandAi, opts) { /* v-brandai · LENH #46: llmChat46 (2 lần, fail-open) · LENH #48: opts.tries (circuit-breaker) */
...
async function llmChat46(body, o) { /* LENH #48 (PB-3): reasoning_effort/max_completion_tokens cho gpt-5/o-series (API từ chối → bỏ field gửi lại) · finish=length → nới trần 1 lần · JSON hỏng thử lại đúng 1 lần · mọi 400 thử 1 lần không json mode · ngân sách thời gian/bài */
...
const usage46 = u => { u = u || {}; const pt = Number(u.prompt_tokens) || 0, ct = Number(u.completion_tokens) || 0; return { prompt: pt, completion: ct, total: Number(u.total_tokens) || (pt + ct), reasoning: Number(u.completion_tokens_details && u.completion_tokens_details.reasoning_tokens) || 0 }; }; /* LENH #48: đếm token suy nghĩ */
...
function heuristicCapped46(post, source, brandAi) { /* LENH #48: truyền Hồ sơ AI brand */
...
/* LENH #48 (PB-5b): heuristic dự phòng biết LĨNH VỰC brand — bigram từ Hồ sơ AI (brands/{code}.ai nganh/dichvu/khach) thay vì chỉ từ khoá agency marketing */
...
function heuristic(text, source, brandAi) {
  ...
  if (bkHit >= 2) score += 25; else if (bkHit === 1) score += 15; // LENH #48: khớp lĩnh vực brand
  ...
  return {
    ...
    service: (brandAi && (brandAi.dichvu || brandAi.nganh)) ? '' : (source.industry ? 'Performance Marketing' : ''), /* LENH #48: không gán dịch vụ agency cho brand khác ngành */
    reply: '',
    _llm: false,
...
export async function scoreLead(post, source, weights, brandAi, opts) { /* v-brandai · LENH #46: LLM hỏng → NÉM LỖI (index.js xếp hàng chấm lại), không còn heuristic ngầm */
  opts = opts || {};
  if (!CFG.LLM_API_KEY) { const h = heuristicCapped46(post, source, brandAi); h._nokey = true; return h; } // LENH #48 (PB-5a): không key → điểm tạm KẸP ≤ 59 (hết 90 điểm nóng từ từ khoá), sweeper không chạy
