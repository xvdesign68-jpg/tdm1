...
R={'--r-sm':'8px','--r-md':'12px','--r-lg':'14px','--r-xl':'22px','--r-card':'20px','--r-full':'999px'}
order=[(k,R.get(k,v)) for k,v in order]
...
# ---- 3. cỡ chữ .5px → thang số nguyên ----
MAP={'8.5':'9','9.5':'10','10.5':'11','11.5':'12','12.5':'13','13.5':'14','14.5':'15','15.5':'16','16.5':'17','17.5':'18','18.5':'19','19.5':'20','21.5':'22','22.5':'22','24.5':'24','25.5':'26'}
def half(m): return 'font-size:%s%spx'%(m.group(1),MAP.get(m.group(2),m.group(2)))
n=0
for f in ['assets/css/app.css','assets/css/pipeline-v2.css','assets/css/tokens.css']+['src/app/'+x for x in sorted(os.listdir(os.path.join(lib.ROOT,'src/app'))) if x.endswith('.js')]+['assets/js/live.js','app.html','assets/js/ui-account.js']:
    s=rd(f); s2,k=re.subn(r'font-size:(\s*)(\d+\.5)px',half,s)
    if k: wr(f,s2); n+=k
lib._log.append('3.halfpx %d'%n)

# ---- 4. bo góc theo thang: 9–13 → md(12) · 14–16 → lg(14) · 17–22 → card(20) · 99/999 → full ----
def rad(m):
    v=int(m.group(2))
    if 9<=v<=13: t='var(--r-md)'
    elif 14<=v<=16: t='var(--r-lg)'
    elif 17<=v<=22: t='var(--r-card)'
    elif v in (99,999): t='var(--r-full)'
    else: return m.group(0)
    return 'border-radius:%s%s'%(m.group(1),t)
n=0
for f in ['assets/css/app.css','assets/css/pipeline-v2.css']:
    s=rd(f); s2,k=re.subn(r'border-radius:(\s*)(\d+)px(?=\s*[;}!])',rad,s)
    if k: wr(f,s2); n+=k
lib._log.append('4.radius %d'%n)
...
# ---- 6. breakpoint gom về 639 / 720 / 1023 / 1180 / 1400 ----
BP={'520':'639','560':'639','640':'639','760':'720','860':'1023','880':'1023','1100':'1180','1140':'1180'}
n=0
for f in ['assets/css/app.css','assets/css/pipeline-v2.css']:
    s=rd(f); s2,k=re.subn(r'@media \(max-width:\s*(\d+)px\)',lambda m:'@media (max-width: %spx)'%BP.get(m.group(1),m.group(1)),s)
    if k: wr(f,s2); n+=k
lib._log.append('6.breakpoints %d'%n)
...
s=rd(p); s2,k=re.subn(r'(\.orb-[12] \{[^}]*?)\s*animation: orbDrift[^;]*;',r'\1',s); wr(p,s2); lib._log.append('8.orb-static %d'%k)
s=rd(p)+r"""
