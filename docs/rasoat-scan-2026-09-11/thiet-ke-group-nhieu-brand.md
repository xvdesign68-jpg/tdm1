...
| 9.19 | `normalizeComment` không map `commentator_profile_url` (dataset dùng tên này, không phải `commenter_*`) → comment-lead 0 % `author_url`; `user_id` 72 ký tự (không phải số trần) | Comment-lead chưa bao giờ kết bạn/inbox | **LỆNH A**: thêm `c.commentator_profile_url` (cắt `?…`), thử `Buffer.from(user_id,'base64')` → nếu ra `\d{10,}` thì `author_uid` |
