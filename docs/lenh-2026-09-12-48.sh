#!/bin/bash
# LỆNH #48 (12/09/2026) — LỆNH A "AI HARDENING" (PB-5 + PB-3 + PB-4 + PB-2 a/b/d + PB-1 + 2 vá scraper) sau rà soát quét→lọc→chấm điểm 11/09 + 8 chốt của anh 12/09
#   (1) lib/scorer.js  llmChat46: reasoning_effort/max_completion_tokens cho gpt-5/o*, timeout 45 s, ngân sách 90 s/bài, JSON hỏng thử lại 1 lần, 400 → thử 1 lần không json; heuristic biết Hồ sơ AI brand (bigram), không gán "Performance Marketing";
#                      thiếu key → điểm tạm KẸP ≤59; replyOnly48 (xin reply 1 lượt, không chấm lại toàn bài); prefilter opts.tries (breaker)
#   (2) lib/scraper.js normalizePost: profile_id → user_url + author_uid + num_comments + gid; normalizeComment: commentator_profile_url (cắt tracking) + user_id pfbid dự phòng → kết bạn/inbox mở cho ~100 % lead
#   (3) index.js       lease ứng viên (score_retry R_<post_id>, gỡ SAU khi ghi lead) · seen ghi bằng create() (chồng lượt → bỏ, không mất bài) · nạp bài chờ bằng transaction · sow46 chỉ lượt lịch
#                      · trần mềm 1.200 s (giữ lease, lượt sau chấm) · bọc lỗi từng bài [pipe] · circuit-breaker 3 hỏng/0 OK · 400 hệ thống → xếp hàng · cửa sổ 15′ [LLM-DOWN] (kích cả 1 bài/lượt) + hysteresis
#                      · [LLM-PRE-DOWN] tầng 1 · sweeper PB-5: sales đang chăm → giữ temp + ai_flag (KHÔNG junk → web không ẩn), không chạy khi thiếu key/breaker, lease 10′, chấm trên MỌI lần chạm
#                      · eKYC bỏ lead LẠNH (zalo_defer, kiểm khi sales chăm) · miss-reply → replyOnly48 · lượt ném lỗi → scans {status:aborted} + system_status/scan + ERROR [SCAN-ABORTED] · nguồn tạm tắt → giữ 6 h
#   (4) lib/config.js  LLM_TRIES · LLM_TIMEOUT_MS 45000 · LLM_POST_BUDGET_MS 90000 · LLM_REASONING low · LLM_MAX_TOKENS 2000 · SCAN_SOFT_DEADLINE_S 1200 · ZALO_CHECK_COLD false (đều tuỳ chọn trong .env)
#   → backup .bak-<TS> 4 file → patch fail-closed NGUYÊN TỬ (54 mốc, đủ mốc cả 4 file mới ghi) → node --check → import test (.env) → gọi thử scoreLead THẬT 1 lần → deploy scheduledScan,manualScan (xích &&)
set -o pipefail
cd ~/firebase-s13/functions || { echo 'KHONG VAO DUOC ~/firebase-s13/functions'; exit 1; }
TS=$(date +%Y%m%d-%H%M%S); echo "backup TS=$TS (giờ UTC)"
cat > _l48_patch.cjs <<'EOF_P48'
...
EOF_A48
node --check _l48_patch.cjs && node --check _l48_after.mjs || { echo 'DỪNG: script lỗi cú pháp (chưa đụng gì)'; exit 1; }
echo "=== (a) backup + patch 4 file ==="
for f in lib/config.js lib/scorer.js lib/scraper.js index.js; do [ -f "$f" ] || { echo "DỪNG: thiếu $f"; exit 1; }; done
cp lib/config.js "lib/config.js.bak-$TS" && cp lib/scorer.js "lib/scorer.js.bak-$TS" && cp lib/scraper.js "lib/scraper.js.bak-$TS" && cp index.js "index.js.bak-$TS" || { echo 'DỪNG: không backup được'; exit 1; }
restore() { cp "lib/config.js.bak-$TS" lib/config.js; cp "lib/scorer.js.bak-$TS" lib/scorer.js; cp "lib/scraper.js.bak-$TS" lib/scraper.js; cp "index.js.bak-$TS" index.js; echo "ĐÃ KHÔI PHỤC 4 file từ .bak-$TS"; }
node _l48_patch.cjs lib/config.js lib/scorer.js lib/scraper.js index.js || { echo 'DỪNG (a): patch không áp — KHÔNG ghi gì. Gửi em output.'; exit 1; }
node --check lib/config.js && node --check lib/scorer.js && node --check lib/scraper.js && node --check index.js || { echo 'DỪNG (a): lỗi cú pháp sau patch'; restore; exit 1; }
echo "marker LENH #48 (config/scorer/scraper/index):"; grep -c "LENH #48" lib/config.js lib/scorer.js lib/scraper.js index.js
echo "=== (b) import test (.env) ==="
set -a; . ./.env; set +a
node --input-type=module -e "const m=await import('./index.js'); const s=await import('./lib/scorer.js'); const r=await import('./lib/scraper.js'); const c=(await import('./lib/config.js')).CFG; const g=s.scoreLead.cfg46; console.log('IMPORT OK · scheduledScan =', typeof m.scheduledScan, '· manualScan =', typeof m.manualScan, '· harvestComments =', typeof r.harvestComments, '· replyOnly48 =', typeof s.scoreLead.replyOnly48, '· cfg46 tries', g.tries, 'timeout', g.timeoutMs, 'ms budget', g.budgetMs, 'ms reasoning', JSON.stringify(g.reasoning), 'maxTokens', g.maxTokens, '· CFG soft', c.SCAN_SOFT_DEADLINE_S, 's · ZALO_CHECK_COLD', c.ZALO_CHECK_COLD, '· model', c.LLM_MODEL, '/', c.LLM_PREFILTER_MODEL)" || { echo 'DỪNG (b): import lỗi sau patch'; restore; exit 1; }
echo "=== (c) gọi thử scoreLead THẬT 1 lần (gpt-5: reasoning_effort + max_completion_tokens; đọc usage.reasoning) — lỗi ở đây KHÔNG chặn deploy ==="
node --input-type=module -e "
const s=await import('./lib/scorer.js'); const t0=Date.now();
const post={kind:'post',post_id:'l48test',author:'Ẩn danh',text:'Cần mua 20kg mực khô loại 1 gửi Hà Nội, ai có báo giá giúp em với 😀'.slice(0,-1)};
try { const ai=await s.scoreLead(post,{industry:'Hải sản',name:'test'},[],{nganh:'Hải sản khô',dichvu:'mực khô, cá khô, tôm khô',khach:'người mua lẻ/sỉ',giong:'thân thiện'}); const u=ai._usage||{}; console.log('LIVE OK ·', ai._model||'', '·', Math.round((Date.now()-t0)/100)/10, 's · hotness', ai.hotness, '· is_real_lead', ai.is_real_lead, '· role', ai.role, '· reply', String(ai.reply||'').length, 'ký tự · token', u.prompt, '+', u.completion, '(reasoning', u.reasoning||0, ') · lượt gọi', s.scoreLead.health46.attempts); }
catch(e){ console.log('LIVE LỖI (kind', e.kind, '):', String(e.message).replace(/sk-[A-Za-z0-9_-]{6,}/g,'sk-…').slice(0,200), '· lượt gọi', s.scoreLead.health46.attempts, '→ vẫn deploy (bản mới xếp hàng chấm lại + cửa sổ 15′ thay vì heuristic)'); }
"
echo "=== (d) deploy scheduledScan,manualScan ==="
cd ~/firebase-s13 && firebase deploy --only functions:scheduledScan,functions:manualScan > /tmp/l48_deploy.log 2>&1; RC=$?; tail -4 /tmp/l48_deploy.log
[ "$RC" = "0" ] && grep -q "Deploy complete" /tmp/l48_deploy.log || { echo 'DEPLOY LỖI — code đã vá vẫn nằm trên đĩa (chưa lên máy chủ). Gửi em /tmp/l48_deploy.log. Muốn quay lại: cd ~/firebase-s13/functions; cp lib/config.js.bak-'"$TS"' lib/config.js; cp lib/scorer.js.bak-'"$TS"' lib/scorer.js; cp lib/scraper.js.bak-'"$TS"' lib/scraper.js; cp index.js.bak-'"$TS"' index.js'; exit 1; }
for f in scheduledScan manualScan; do gcloud functions describe "$f" --region asia-southeast1 --gen2 --format='value(name,state,updateTime)' 2>/dev/null | sed 's|projects/smartlead-z15/locations/asia-southeast1/functions/||' || echo "(describe $f bỏ qua)"; done
echo "=== XONG KHỐI 1 (exit=0) — KHỐI 2 (sau ≥ 15′, có bài mới): cd ~/firebase-s13/functions && node _l48_after.mjs ==="
