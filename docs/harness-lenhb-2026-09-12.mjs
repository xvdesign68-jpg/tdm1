...
{ fresh(); clockOff = 0; BD.recordsFor = i => (i.url === G1 ? [rec('s1', { content: 'Bán mực khô loại 1 giá tốt' }), rec('s2')] : []); route.main = (body) => good(/Bán mực khô loại 1/.test(JSON.stringify(body)) ? Object.assign({}, LEAD, { is_real_lead: false, role: 'seller', hotness: 20 }) : LEAD);
  await ixm.scanAll('scheduled'); clockOff += 130e3; await ixm.scanAll('scheduled'); const g = gsDoc(GK) || {}; const L = leads();
  if (process.env.LB_DBG) OUT('DBG C15', JSON.stringify({ watch: g.watch, ids: L.map(l => l.id), seen: seenOf('s1') }));
  ok(L.length === 2 && L.every(l => l.post_id === 's2') && g.watch && g.watch.s1 && g.watch.s2 && (seenOf('s1').brands.b1 === 'seller'), 'C15 (rà) bài người bán (không lead) 0 bình luận → vẫn vào watch_posts (bình luận đến sau dưới bài người bán = comment-lead 44 %); seen.brands = seller'); }
...
{ fresh(); clockOff = 0; await F.db.collection('sources').doc('src_b1_dup').set({ name: 'Hải sản B1 (trùng)', url: 'https://www.facebook.com/groups/1189400231607822?ref=share', brand: 'b1', active: true, industry: 'Hải sản', sharedAt: Date.now() });
  BD.recordsFor = i => (i.url === G1 ? [rec('u1')] : []); await ixm.scanAll('scheduled'); clockOff += 130e3; calls = []; await ixm.scanAll('scheduled'); const L = leads(); const last = scansAll()[1]; const rd = (last.bySource || []).find(r => r.source_id === 'src_b1_dup');
  ...
  ok(L.length === 2 && L.map(l => l.id).sort().join(',') === 'L_u1__b1,L_u1__b2' && last.dupSources === 1 && rd && /trùng nguồn cùng brand/.test(rd.error || '') && calls.filter(c => c.body.model === 'gpt-5.6-sol').length === 2 && !logHas(/ghi lease lỗi/) && logHas(/\[SOURCE-DUP\]/), 'C23 (rà) nguồn trùng brand+group → bỏ qua (row.error + scans.dupSources + WARNING), 2 lead đúng, AI 2 lượt, lease không lỗi batch'); }
