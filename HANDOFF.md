...
3. ☐ **Zip frontend MỚI NHẤT** = `smartleads17deploy-v119-65.zip` (= v119-60 + đợt sửa UI theo báo cáo rà soát, ĐÃ BỎ phần design-system làm xấu đi, trả lại khung hotline + cặp font anh chốt Plus Jakarta Sans (tiêu đề/nút/nhãn) + Be Vietnam Pro (thân chữ), JetBrains Mono cho mã + kênh realtime tự đăng ký lại 4 lần sau permission-denied thoáng qua (LỆNH #38): menu 4 nhóm tên Việt, gỡ Tích hợp/hotline/khối demo, Bảng điều khiển sắp lại + dải trạng thái, errMsg + hộp thoại app, hết chữ kỹ thuật/ALL-CAPS, 203 emoji → SVG, menu ⋯, bóng cuộn bảng, KPI 2 cột mobile, :root duy nhất + bỏ Google Fonts + dọn CSS chết; mascot giữ nguyên; smoke 108/108 — mục ★ v119-61 CLAUDE.md). Bản riêng `smartleads17deploy-v120-esm-m.zip` (ES module, cùng thay đổi) deploy sau khi v119-63 ổn.
4. ☐ **`worker.mjs` bản `2026-09-06d`** (gửi anh 06/09 tối: watchdog HUỶ CỨNG phiên nick treo — hết zombie double-act; mục 06d `docs/worker-2026-09-06-refund-capture.md`; harness `docs/harness-worker-zombie-2026-09-06.mjs` 13/13; config.json không đổi; chép đè MỌI VPS + run.bat). Trước đó ☑ (anh ĐÃ chép VPS 06/09) `2026-09-06c` (gửi anh 06/09 cùng LỆNH #34: `stampExtra` ghi `leads.outreach.content` = meta nội dung + `comment_at/inbox_at` cho card Hiệu quả nội dung — mục 06c `docs/worker-2026-09-06-refund-capture.md`; config.json không đổi; trước đó 2026-09-06b: comment-lead đi API nội bộ — tym + trả lời bình luận qua /api/graphql/, `config.json.graphql.commentDocId/replyDocId = 28781864408106143`; 06 = hoàn van kết bạn khi bỏ qua + `--capture` — `docs/worker-2026-09-06-refund-capture.md`; trước đó 2026-09-05c gửi 05/09: 05 = giải mã uid chủ trang + checkReplies phủ mọi lead; 05b = nhãn funnel + kiểm ngôn ngữ FB (nick phải để Tiếng Việt) + chờ vẽ + skip nội dung không xem được; 05c = trả lời comment-lead nhận ô soạn theo FOCUS/cấu trúc, nút Thích icon — `docs/worker-2026-09-05b-comment-reply.md`; trước đó 2026-09-04b Safety Score) — nếu định sửa worker.
