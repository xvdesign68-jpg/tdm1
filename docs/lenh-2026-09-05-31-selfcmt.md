...
## ★ KHỐI 4b (06/09 ~06:35 VN) — KHÔNG chặn nhầm, KHÔNG cần #31c
- Hồ sơ AI: `hscl-01` khách = người kinh doanh nhập sỉ hải sản khô (automation BẬT, content.mode reply); `z15mrc-tts-1` khách = sinh viên/mới tốt nghiệp tìm thực tập; `z15mrc-tuyendung1` khách = ỨNG VIÊN đang tìm việc (Hà Nội/Hải Phòng). ⇒ với 2 brand tuyển dụng, người ĐĂNG TIN TUYỂN là bên cung (đối thủ giành ứng viên), không phải khách → AI xếp `seller` ca "Cần tìm 2 bạn đi học và thi… công 1 triệu/người" (điểm 5 junk) là ĐÚNG.
- 5 decision `seller` sau deploy (05:20–06:29 VN) đều điểm 3–5 junk, đúng người bán/nhà tuyển dụng/khoá học khởi nghiệp (hscl-01 ×3, tuyendung1 ×1, test-agency ×1).
- 4 lead `role rỗng` đều có detected_at 01:23–04:59 VN = TRƯỚC deploy 05:12 VN → không phải lỗi; cả 4 là người tìm việc (buyer đúng của brand tuyển dụng).
- **Giới hạn ghi nhớ**: SYS_RULES có câu "TUYỂN NGƯỜI cho chính họ = seller" — đúng với mọi brand hiện có; nếu sau này có brand phục vụ NHÀ TUYỂN DỤNG (dịch vụ tuyển dụng cho doanh nghiệp) thì phải sửa câu này thành tương đối theo brand (#31c lúc đó).
- Kế: sáng ban ngày chạy lại `node _l31_after.mjs 6` (kỳ vọng `self_comment > 0`, lead mới có vai `buyer`); KHỐI 3 rescore tuỳ chọn; deploy zip v119-52 để thấy chip 🏪/👤.
