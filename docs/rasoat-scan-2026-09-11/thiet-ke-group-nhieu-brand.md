...
| 9.18 | **`profile_id` (uid số 16 ký tự) có trong record bài nhưng `normalizePost` không map** (KHỐI 0: dataset không có `user_url`) → ~31 % lead có `author_url`, 0 uid số | Automation kết bạn/inbox chỉ chạy 1/3 lead, DOM thay vì API | **LỆNH A**: `normalizePost` thêm `user_url = profile.php?id=<profile_id>` (khi toàn số) + `author_uid`; lead ghi `author_uid` → engine `hasProfile` ≈ 100 %, worker `gqlFriend` theo uid số, inbox `messenger.com/t/<uid>`; kiểm `profile_id` là số trong KHỐI kiểm A |
| 9.19 | `normalizeComment` không map `commentator_profile_url` (dataset dùng tên này, không phải `commenter_*`; giá trị 454 ký tự = URL + tracking `__cft__`) → comment-lead 0 % `author_url`; `user_id` 72 ký tự = **pfbid** (không phải base64 `user:<uid>` 29 ký tự) | Comment-lead chưa bao giờ kết bạn/inbox | **LỆNH A**: `author_url` = `commentator_profile_url` cắt `?…`; `author_uid` = số rút từ `/user/<uid>/` · `?id=<uid>` · `/people/<tên>/<uid>/`; trống → `profile.php?id=<pfbid>` (DOM mở được, không uid) |
| 9.20 | `num_comments` CÓ trong record bài (mẫu = 0 lúc phát hiện); snapshot bình luận cho bài 0 comment trả 3/3 record lỗi | §9.4 + câu 6 watch_posts | Gieo bình luận CHỈ khi `num_comments > 0` lúc gặt; bài `num_comments = 0` → vào `watch_posts` (≤ 5 bài/nguồn, 2 lần/ngày, 2 ngày) re-fetch record bài, tăng → gieo comment; đo record comment rỗng trước/sau |
| 9.21 | `tagLeadBrand` (codebase2, deploy 08-11 gen2) = `onDocumentCreated leads` → `sources.where('name','==',lead.source).limit(1)` → set brand nếu lead chưa có; race với birthstamp inline s13 (~50 ms) | Group dùng chung + source doc trùng TÊN → gán brand SAI | B: lead create ghi `brand_hint`+`source_id`; inline stamp dùng `brand_hint`; LỆNH C vá tagLeadBrand dòng 75 `const brand = hint || await getBrandForSource(sourceName)` + deploy `--only functions:tagLeadBrand` từ codebase2 (mốc text đã có: dòng 75 `const brand = await getBrandForSource(sourceName);` duy nhất; thêm `brand_tagged_by: hint ? 'brand_hint' : 'tagLeadBrand'`) |
