...
#   → backup .bak-<TS> → patch fail-closed NGUYÊN TỬ (73 mốc, đủ 7 file mới ghi) → node --check → Rules → import test (.env) → deploy 8 function + rules (xích &&) → tagLeadBrand (codebase2, lỗi = cảnh báo)
set -o pipefail
...
{ const s = ss.exists ? ss.data() : {}; console.log('1. system_status/scan: phase=' + (s.phase || '—') + ' · lượt gần nhất ' + hm(s.lastRunAt) + ' (' + ago(s.lastRunAt) + ') ' + Math.round((s.lastDurationMs || 0) / 1000) + ' s · groups ' + (s.groups ?? '—') + ' · sown ' + (s.sown ?? '—') + ' · harvested ' + (s.harvested ?? '—') + ' · dupLead ' + (s.dupLead ?? '—') + ' · skipRuns ' + (s.skipRuns || 0) + ' (lượt thuần skip không ghi scans; lần cuối ' + hm(s.lastSkipAt) + ') · việc phụ lần cuối ' + hm(s.lastHousekeepingAt) + ' (' + ago(s.lastHousekeepingAt) + ', nhịp ≥5′)' + (s.phase === 'aborted' ? ' ⚠ ABORTED: ' + mask(s.lastError) : ''));
...
{ const pb = pend.filter(p => p.id.startsWith('PB_')), c = pend.filter(p => p.id.startsWith('C_')), p0 = pend.filter(p => p.id.startsWith('P_'));
  console.log('3. pending_snapshots: gộp PB_ ' + pb.length + ' (' + pb.map(p => Math.round((now - ms(p.t)) / 60000) + '′/' + (p.groups || []).length + 'g' + (p.ready ? '✓' : '')).join(' ') + ') · bình luận C_ ' + c.length + ' · P_ cũ (đường nick/không group) ' + p0.length + (pend.some(p => now - ms(p.t) > 2 * 3600e3) ? ' ⚠ có snapshot >2 h (sẽ bị bỏ)' : '')); }
...
for f in $FILES; do cp "$f" "$f.bak-$TS" || { echo "DỪNG: không backup được $f"; exit 1; }; done
cp ../firestore.rules "../firestore.rules.bak-$TS" || { echo 'DỪNG: không backup được firestore.rules'; exit 1; }
[ -f sources.js ] && cp sources.js "sources.js.bak-$TS"
restore() { for f in $FILES; do cp "$f.bak-$TS" "$f"; done; cp "../firestore.rules.bak-$TS" ../firestore.rules; [ -f "sources.js.bak-$TS" ] && cp "sources.js.bak-$TS" sources.js || rm -f sources.js; echo "ĐÃ KHÔI PHỤC 7 file + firestore.rules từ .bak-$TS"; }
...
[ "$RC" = "0" ] && grep -q "Deploy complete" /tmp/lb_deploy.log || { echo 'DEPLOY LỖI — code đã vá vẫn nằm trên đĩa (chưa lên máy chủ). Gửi em /tmp/lb_deploy.log. Muốn quay lại: cd ~/firebase-s13/functions; for f in lib/config.js lib/scraper.js index.js outreach.js stats.js scanstats.js lib/multitouch.js; do cp "$f.bak-'"$TS"'" "$f"; done; rm -f sources.js; cp ~/firebase-s13/firestore.rules.bak-'"$TS"' ~/firebase-s13/firestore.rules'; exit 1; }
for f in scheduledScan manualScan outreachTick statsOnLead scanStatsOnRun createSource sourceOnWrite bdReady; do gcloud functions describe "$f" --region asia-southeast1 --gen2 --format='value(name,state,updateTime)' 2>/dev/null | sed 's|projects/smartlead-z15/locations/asia-southeast1/functions/||' || echo "(describe $f bỏ qua)"; done
