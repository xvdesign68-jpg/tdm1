...
/* ---- Pha 3b: TẦNG 2 — model THÔNG MINH chấm sâu + viết phản hồi (song song) ---- */
  ...
  await mapPool(toScore, CFG.SCORE_CONCURRENCY || 6, async (x) => {
    ...
    if (ai._llm !== false && !(ai.reply && String(ai.reply).trim())) { // LENH #46: đang dùng dự phòng thì không gọi lại
      for (let __rt = 0; __rt < 2; __rt++) {
        try {
          const __ai2 = await scoreLead(x.post, x.effSrc, config.weights, await __brandAiOf(x.effSrc), { tries: 2 }); // LENH #46
...
export const manualScan = onRequest({ cors: true }, async (req, res) => {
  ...
  if (b.mode === 'backfill') {
    ...
    const sum = await scanAll('backfill', opts);
    res.json({ ok: true, mode: 'backfill', range: { label: opts.rangeLabel, start, end }, summary: sum });
    return;
  }
  const sum = await scanAll('manual', { jobId, force: !!b.force });
