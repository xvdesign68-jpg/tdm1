...
- **Code app THẬT KHÔNG nằm trong git.** Anh phải **đưa lại 3 thứ** cho phiên mới: (1) **zip frontend mới nhất `smartleads17deploy-v119-89.zip`** (+ bản riêng `v120-esm-am`), (2) **`worker.mjs` bản `2026-09-11`**, (3) `config.json` của worker. Gói **`smartlead-handoff-2026-09-11.zip`** (em gửi 11/09 sáng) chứa bản CŨ HƠN 1 nấc (v119-87 / 06d) — zip v119-89 (11/09 tối) + worker 2026-09-11 gửi rời, ưu tiên bản mới; docs/ trong git luôn mới nhất.
...
- **Việc đang dở lúc bàn giao**: rà soát automation "Tiếp cận" 10/09 đã có số thật (LỆNH #43/#43b/#44a) → **LỆNH #44 engine ĐÃ CHẠY + nghiệm thu PASS 11/09 11:29 VN (rev `outreachtick-00030-bub`); worker `2026-09-11` + zip `v119-88` ĐÃ LÀM + gửi 11/09 chiều — CHỜ ANH NGHIỆM THU** (mục 6b, việc số 1).
...
### 5.1 Frontend
- **Zip mới nhất cần deploy: `v119-89`** (11/09 tối, `m31.py`) = v119-88 + FE đi cùng **LỆNH #46** (lead "Điểm tạm – AI chưa chấm"): chip "Điểm tạm – AI sẽ chấm lại" mọi vai trò (thẻ + modal), Nên gọi tiếp theo −30, Bài đã quét "Chờ AI chấm lại", `system_status/llm` → Cảnh báo hệ thống + thẻ vàng (Overview/Lịch sử quét) + chip thanh nhịp quét "AI chấm điểm lỗi"; bản riêng **`v120-esm-am`**. Smoke **135/135** cả 2 cây; tái lập byte-identical. Ảnh `docs/fe-2026-09-07-v119-61/shots-v119-89/`.
- Trước đó **`v119-88`** (11/09 chiều, `m30.py`) = v119-87 + zip FE nhỏ Đợt 1 automation: nick FB realtime cho super (`fb_accounts` onSnapshot, chữ ký bỏ trường ghi mỗi tick), van ⚙ kẹp trần cứng 80/30/20/15 đọc từ heartbeat VPS, tooltip Safety nói rõ lý do (`safetyWhy`), thẻ brand "BẬT · 0/N nick" + "k/N chạy được" + dòng máy chủ `system_status/outreach`, Cảnh báo hệ thống thêm dòng brand BẬT 0 nick, chip máy trên thẻ lead ghi tên nick từ `pid`, nhãn log "Chạy thử"; bản riêng **`v120-esm-al`**. Smoke **133/133** cả 2 cây; boot-stub OK; tái lập byte-identical. Ảnh `docs/fe-2026-09-07-v119-61/shots-outreach-v119-88/`.
- Trước đó **`v119-87`** (10/09 sáng) = v119-86 + trang Giá trị & ROI cho Super Admin toàn cảnh mọi brand (CPL 2 cách A/B, `brands.roi.cplBasis`, phí gói theo `brands.plan` hoặc `feeSrc custom`, bảng 17 cột cùng số Bảng brand, "Xem như khách"); bản riêng **`v120-esm-ak`**. Smoke **129/129** cả 2 cây. Anh chưa xác nhận đã deploy + chưa nhận xét bố cục ROI (ảnh `docs/fe-2026-09-07-v119-61/shots-roi-v119-87/`).
- Chuỗi gần nhất (mỗi bản = 1 script `mNN.py`, README trong `docs/fe-2026-09-07-v119-61/`): v119-80 KPI 14 ngày theo `daily_stats` → 81 hàng KPI dễ hiểu → 82 KPI 4 ô đều → 83 Pipeline "Việc cần chú ý" gom nhóm → 84 toast mobile → 85 chữ số 1 font (`--font-num`) → 86 brand realtime cho super (`brands` onSnapshot) → 87 ROI super → 88 zip FE nhỏ Đợt 1 automation. Anh đã deploy tới **v119-80** (xác nhận 09/09); v119-81…87 bỏ qua, deploy thẳng 88.
...
### 5.2 Worker VPS
- **Bản mới nhất cần chép: `worker.mjs 2026-09-11`** (gửi 11/09 chiều; `docs/worker-2026-09-11.md` + patch `docs/worker-2026-09-11-patch.cjs` áp lên 06d + harness `docs/harness-worker-2026-09-11.mjs` 67/67 + 8 harness cũ): R-15 bộ biến comment bài theo capture 06/09 (hết fail bài group gid số) + `ambiguous` kiểm hiển thị trước DOM (chống đăng 2 lần) + DOM bấm nút "Bình luận" nhận ô theo focus · R-9 lỗi giao diện (`uiFailCount`) KHÔNG trừ Safety, lỗi FB đếm 1 lần/phiên, decay theo ngày tròn, `safetyWhy`, di trú tự động (t7 lên lại điểm) · R-8 `markDone` trước log + Ctrl+C tắt êm · R-2 `replyLoop` đọc inbox độc lập + backoff `replyNextAt` · R-1 `preflight` + kiểm người thật trước mỗi bước · `dryRun` · hoàn van kẹp ≥0 cùng ngày · `nextMorningVN` 0–8 h → 8 h hôm nay. Khoá config.json mới (tuỳ chọn): `dryRun`, `stopGraceMs`, `replyLoop`, `inbox.backoffH`. Nghiệm thu theo doc §4 (chép → dryRun → thật 1 nick).
- **Đang chạy trên VPS: `worker.mjs 2026-09-06d`** trên `vps-1` (1 VPS, `maxConcurrent 1`, `inbox.enabled true`, worker online). **Nick: 0/6 sống** — 4 nick needLogin + tắt (`apk1g9yjdv/apk1ge6dka/apk1gehnnq/apk1gm2jd2`), nick t7 `apk1gm6por` bị Safety tự tắt 09/09 (Safety 28 vì 43 lỗi selector — xem 6b). **Khuyên: chưa bật lại nick tới khi có worker mới (Đợt 1).**
...
0. **LỆNH #46 XONG 11/09 (KHỐI 1 17:09 + KHỐI 2 nghiệm thu PASS 17:28 VN; backlog 252 lead điểm tạm đang được sweeper chấm lại 12/lượt ≈ xong 19:00 VN — chạy lại `node _l46_after.mjs` xác nhận "còn lại" ≈ 0; nếu còn > ~80 lead > 30 ngày → LỆNH #47 index)** — lead "Điểm tạm – AI chưa chấm" (mục ★ LỆNH #45 + #46 trong CLAUDE.md; `docs/lenh-2026-09-11-46.md`): LỆNH #45 đã chạy → gốc = OpenAI 500 từng đợt + retry 1 lần → heuristic 90 NÓNG; #46 = llmChat46 thử lại 4 lần + xếp hàng chấm lại `score_retry` + sweeper chấm lại lead điểm tạm + engine/push bỏ qua + giám sát `system_status/llm`. Khối bash dán chat (KHỐI 1) → deploy 4 function → KHỐI 2 `node _l46_after.mjs` sau ≥ 15′; kèm zip v119-89.
0a. **12/09: ANH ĐÃ CHỐT 8 câu (4 = hướng B "1 group nhiều brand", 5 = tốc độ "không bỏ lỡ lead, nhanh nhất") — thiết kế bản 2 `docs/rasoat-scan-2026-09-11/thiet-ke-group-nhieu-brand.md` (§8 sau phản biện) — CHỜ ANH CHẠY LỆNH #47 chỉ đọc (`docs/lenh-2026-09-12-47.md/.sh`) + cho 3 số BrightData (trần API, đơn giá, notify) → em soạn A0 → A → B → C → zip.** Mục ★ ANH ĐÃ CHỐT 8 CÂU trong CLAUDE.md.
0b. **RÀ SOÁT QUÉT→LỌC→CHẤM ĐIỂM→NHẬN DIỆN LEAD 11/09 tối (chỉ đánh giá) — đã chốt 12/09 (xem 0a)** (mục ★ cùng tên trong CLAUDE.md; artifact https://claude.ai/code/artifact/18dfe84a-fa8d-4a52-99f8-d0b715cd225f; `docs/rasoat-scan-2026-09-11/`): 95 phát hiện (4 Cao: N4-3 sweeper #46 giấu lead đang chăm · F-A2/N3-12 bộ lọc toàn cục "ma" · Q-3 bình luận chỉ lấy 1 lần) + 23 đề xuất; Đợt 1 (bản 2, sau giám khảo 2 + critic) = LỆNH A0 (PB-5 + PB-2a, deploy trước) · A (PB-3 + PB-4 + PB-2bd, có A/B) · C (PB-10 + PB-6 sau khi chốt câu 1 + Rules 2 field) · D (PB-9 rút gọn + PB-12 đo/brand ctx) · zip FE (PA-4 + PC-6 + PA-11 + PB-12 chặn trùng tên). Chốt xong → 6 LỆNH chỉ đọc R1–R6 (dump 7 file backend chưa từng dump) rồi mới soạn patch.
1. **Chốt Đợt 1 automation** (mục ★ RÀ SOÁT AUTOMATION 10/09 trong CLAUDE.md). **Cập nhật 11/09**: gốc W-19 đã xác định từ capture 06/09 (không cần (c)); LỆNH #44a đã chạy; **LỆNH #44 engine ĐÃ CHẠY OK 11/09 11:09 VN + KHỐI 2 PASS 11:29 VN** (`docs/lenh-2026-09-11-44.md` — sweeper expired 11 / orphan 1, hscl-01 0/6 nick sống, ERROR→WARNING đúng, BrightData sập 3′ 10/09 đã hồi); **worker `2026-09-11` + zip `v119-88` / `v120-esm-al` ĐÃ LÀM + gửi 11/09 chiều → CHỜ ANH NGHIỆM THU** theo `docs/worker-2026-09-11.md` §4: chép worker (giữ config.json; Ctrl+C 1 lần = tắt êm) → đăng nhập lại 1 nick + ✓ Đã xử lý → `"dryRun": true` chạy thử (log `[DRY] … ✓`) → tắt dryRun chạy thật 1 nick (`comment API: OK capture`) → van hscl-01 về 40/12/10/8 → deploy zip v119-88. (Trước đó chờ 3 bằng chứng từ VPS: (a) đếm bình luận của nick t7 trên bài của 2 lead `zt4t59NCvZigXs9tlRmF` / `VMjR1TEGGOOtcNIpvJoS` (nghi đăng trùng W-20); (b) 1 ảnh `errors/*__comment_0.png`; (c) `node worker.mjs --capture k1gm6por` rồi tự tay bình luận cấp 1 lên 1 bài group gid SỐ → `capture-<ts>.log` (lấy bộ biến đúng cho R-15). Sau đó làm: **worker 2026-09-10** (R-15 bình luận bài bấm nút "Bình luận" + nhận ô theo focus + dò doc_id lỗi thời · R-9 tách lỗi FB ↔ lỗi giao diện, Safety không trừ vì selector, đếm theo phiên, decay đúng · R-8 ghi tiến độ nguyên tử + SIGINT êm + API mơ hồ kiểm trước DOM · R-2 phiên đọc inbox độc lập + backoff · `dryRun`) + **LỆNH #44 engine** (E-16 due-nick lọc needLogin/safetyPaused · R-6 sweeper thread mồ côi · E-17 `effectiveCaps=min(brand,hardCap)` · R-1 lọc lead có người thật + trigger tắt thread · E-4 reserve lại van khi re-enqueue khác ngày) + **zip FE nhỏ** (F-1 nick realtime · F-2 trần van + lý do Safety · F-6/F-7 tên nick + chip "BẬT nhưng 0/N nick"). Trước đó: đưa van hscl-01 về Cân bằng 40/12/10/8; sau khi worker mới chạy mới bấm "✓ Đã xử lý" cho t7.)
2. **Deploy zip v119-89** (Cmd+Shift+R; v119-87/88 bỏ qua) rồi bản riêng `v120-esm-am`; hỏi anh bố cục ROI super (v119-87) có ưng không.
