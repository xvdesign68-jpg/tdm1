/* _l41_recount.mjs — (1) CHẨN ĐOÁN: 6 lead mới nhất (kiểu dữ liệu brand/temp/score/detected_at) + so daily_stats {new,hot,junk} 4 ngày gần nhất với số đếm lại từ leads;
   ...
   Dùng: node _l41_recount.mjs [--dry] [--days=60]   (đặt trong ~/firebase-s13/functions, cạnh stats.js) */
...
const isMain = process.argv[1] && /_l41_recount\.mjs$/.test(process.argv[1]);
if (isMain) {
  ...
  const recent = await db.collection('leads').orderBy('detected_at', 'desc').limit(6).select('brand', 'temp', 'score', 'detected_at', 'stage', 'source').get();
  ...
  for (;;) { let q = db.collection('leads').orderBy('__name__').select('brand', 'temp', 'score', 'detected_at').limit(500); if (last) q = q.startAfter(last); const snap = await q.get(); if (snap.empty) break;
  ...
  console.log((DRY ? '[DRY] ' : '') + 'recount new/hot/warm/cold/junk: quét ' + scanned + ' lead (bỏ qua: không brand ' + skipped.noBrand + ', không detected_at ' + skipped.noDet + ') → ' + keys.length + ' doc daily_stats từ ' + from + ' (' + DAYS + ' ngày)');
