# bash — LỆNH D · KHỐI 2b (không shebang để tránh history-expand khi dán Cloud Shell)
# LỆNH D · KHỐI 2b (13/09/2026) — CHỈ ĐỌC: ghi đè _ld_after.mjs bằng BẢN 2 (vá lỗi select() thiếu field orderBy ở bản 1) rồi chạy nghiệm thu.
# Dán: cat > /tmp/ld2b.sh <<'EOF' … EOF ; bash /tmp/ld2b.sh   (hoặc chạy thẳng file này). Không ghi gì lên Firestore.
set -u
cd ~/firebase-s13/functions || { echo 'DỪNG: không vào được ~/firebase-s13/functions'; exit 1; }
test -f _ld_after.mjs && cp -n _ld_after.mjs _ld_after.v1.bak.mjs 2>/dev/null
cat > _ld_after.mjs <<'EOF_AFTER2'
...
EOF_AFTER2
node --check _ld_after.mjs || { echo 'DỪNG: _ld_after.mjs lỗi cú pháp'; exit 1; }
echo "sha256 _ld_after.mjs: $(sha256sum _ld_after.mjs | cut -c1-16)…"
node _ld_after.mjs
echo "exit=$?"
