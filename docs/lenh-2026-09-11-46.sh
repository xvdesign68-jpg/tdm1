#!/bin/bash
# LỆNH #46 (11/09/2026) — LEAD "ĐIỂM TẠM – AI CHƯA CHẤM" VÁ TRIỆT ĐỂ (theo LỆNH #45: OpenAI 500 từng đợt + retry 1 lần/1,5 s → heuristic → lead rác 90 NÓNG; phụ: 400 Invalid body do emoji cắt đôi)
#   (1) lib/scorer.js llmChat46: sanitize · timeout 60 s · thử lại 4 lần 2/5/12 s (Retry-After) · KHÔNG thử lại auth/quota/model/400 · lỗi có kind · health46; scoreLead ném lỗi thay vì heuristic ngầm
#   (2) index.js: LLM hỏng → score_retry (3′→10′→30′→1h→3h→6h, ≤6 lần/24h) · nạp lại đầu Pha 3b · quá hạn → lead dự phòng KẸP ≤59 · sweeper chấm lại lead ai_scored:false ≤12/lượt · [LLM-DOWN]/[LLM-UP] + system_status/llm · scans llm*
#   (3) outreach.js: engine không bốc lead ai_scored:false   (4) push.js: không push "Lead nóng mới" điểm tạm; AI chấm lại thành nóng → push
#   → backup .bak-<TS> → patch fail-closed NGUYÊN TỬ 4 file → node --check → import test (.env) → gọi thử scoreLead THẬT 1 lần → deploy scheduledScan,manualScan,outreachTick,pushOnLead (xích &&)
set -o pipefail
cd ~/firebase-s13/functions || { echo 'KHONG VAO DUOC ~/firebase-s13/functions'; exit 1; }
TS=$(date +%Y%m%d-%H%M%S); echo "backup TS=$TS (giờ UTC)"
cat > _l46_patch.cjs <<'EOF_P46'
...
EOF_A46
node --check _l46_patch.cjs && node --check _l46_after.mjs || { echo 'DỪNG: script lỗi cú pháp (chưa đụng gì)'; exit 1; }
echo "=== (a) backup + patch 4 file ==="
for f in lib/scorer.js index.js outreach.js push.js; do [ -f "$f" ] || { echo "DỪNG: thiếu $f"; exit 1; }; done
cp lib/scorer.js "lib/scorer.js.bak-$TS" && cp index.js "index.js.bak-$TS" && cp outreach.js "outreach.js.bak-$TS" && cp push.js "push.js.bak-$TS" || { echo 'DỪNG: không backup được'; exit 1; }
restore() { cp "lib/scorer.js.bak-$TS" lib/scorer.js; cp "index.js.bak-$TS" index.js; cp "outreach.js.bak-$TS" outreach.js; cp "push.js.bak-$TS" push.js; echo "ĐÃ KHÔI PHỤC 4 file từ .bak-$TS"; }
node _l46_patch.cjs lib/scorer.js index.js outreach.js push.js || { echo 'DỪNG (a): patch không áp — KHÔNG ghi gì. Gửi em output.'; exit 1; }
node --check lib/scorer.js && node --check index.js && node --check outreach.js && node --check push.js || { echo 'DỪNG (a): lỗi cú pháp sau patch'; restore; exit 1; }
echo "marker LENH #46:"; grep -c "LENH #46" lib/scorer.js index.js outreach.js push.js
echo "=== (b) import test (.env) ==="
set -a; . ./.env; set +a
node --input-type=module -e "const m=await import('./index.js'); const s=await import('./lib/scorer.js'); const o=await import('./outreach.js'); const p=await import('./push.js'); console.log('IMPORT OK · scheduledScan =', typeof m.scheduledScan, '· manualScan =', typeof m.manualScan, '· outreachTick =', typeof o.outreachTick, '· pushOnLead =', typeof p.pushOnLead, '· scoreLead.health46 =', typeof s.scoreLead.health46, '· tries', s.scoreLead.cfg46.tries, '· timeout', s.scoreLead.cfg46.timeoutMs, 'ms')" || { echo 'DỪNG (b): import lỗi'; restore; exit 1; }
echo "=== (c) gọi thử scoreLead THẬT 1 lần — bài có emoji bị cắt đôi (chứng minh sanitize + thử lại); lỗi ở đây KHÔNG chặn deploy ==="
node --input-type=module -e "
const s=await import('./lib/scorer.js'); const t0=Date.now();
const post={kind:'post',post_id:'l46test',author:'Ẩn danh',text:'Mô hình ngoài trời có mái che kéo ổn k ae, ae cho lời khuyên ạ. Thuê mb chỉ 5 triệu trung tâm thị xã 😀'.slice(0,-1)};
try { const ai=await s.scoreLead(post,{industry:'',name:'test'},[],null); console.log('LIVE OK ·', ai._model||'', '·', Math.round((Date.now()-t0)/100)/10, 's · hotness', ai.hotness, '· is_real_lead', ai.is_real_lead, '· role', ai.role, '· reply', String(ai.reply||'').length, 'ký tự · lượt gọi', s.scoreLead.health46.attempts, '(hỏng', s.scoreLead.health46.attemptsFail + ')'); }
catch(e){ console.log('LIVE LỖI (kind', e.kind, '):', String(e.message).slice(0,200), '· lượt gọi', s.scoreLead.health46.attempts, '→ vẫn deploy (bản mới xếp hàng chấm lại thay vì heuristic)'); }
"
echo "=== (d) deploy scheduledScan,manualScan,outreachTick,pushOnLead ==="
cd ~/firebase-s13 && firebase deploy --only functions:scheduledScan,functions:manualScan,functions:outreachTick,functions:pushOnLead > /tmp/l46_deploy.log 2>&1; RC=$?; tail -4 /tmp/l46_deploy.log
[ "$RC" = "0" ] && grep -q "Deploy complete" /tmp/l46_deploy.log || { echo 'DEPLOY LỖI — code đã vá vẫn nằm trên đĩa (chưa lên máy chủ). Gửi em /tmp/l46_deploy.log. Muốn quay lại: cd ~/firebase-s13/functions; cp lib/scorer.js.bak-'"$TS"' lib/scorer.js; cp index.js.bak-'"$TS"' index.js; cp outreach.js.bak-'"$TS"' outreach.js; cp push.js.bak-'"$TS"' push.js'; exit 1; }
for f in scheduledScan manualScan outreachTick pushOnLead; do gcloud functions describe "$f" --region asia-southeast1 --gen2 --format='value(name,state,updateTime)' 2>/dev/null | sed 's|projects/smartlead-z15/locations/asia-southeast1/functions/||' || echo "(describe $f bỏ qua)"; done
echo "=== XONG KHỐI 1 (exit=0) — KHỐI 2 (sau ≥ 15′, có bài mới): cd ~/firebase-s13/functions && node _l46_after.mjs ==="
